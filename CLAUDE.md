...
- **Duplicate checks happen before the import confirm call and cannot be overridden.** The wizard compares the resolved account plus the inclusive parsed date range through `/api/statements/check-overlap` or `/api/credit-payments/check-overlap`; bank/debit, billed card, and broker portfolio archives all stop on the same **Statement Already Exists** dialog before transactions, balances, holdings, or archive records change. Both create and update APIs enforce the same account + overlap rule with HTTP 409, so bypassing the UI still cannot create another record. Credit Payments store exact `period_from` / `period_to`; legacy/manual records without them use the previous cutover-based fallback.
