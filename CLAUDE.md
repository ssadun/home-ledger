...
- **Deleting a CreditPayment deletes its card spendings.** Rows linked through `transactions.credit_payment_id` are imported statement rows owned by that Card Payment, so `DELETE /api/credit-payments/{id}` removes them together with the record and stored statement file. This is intentionally different from non-credit `Statement` delete, which keeps bank-account movements and only clears `statement_id`.
