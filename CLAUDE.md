...
### Backend tests
`backend/tests/` — pytest. `test_bank_import_fixtures.py` holds the golden fixtures: it parses the real sample statements in `import/` (one per format in the registry below) and locks in row counts, totals, account identity, and the classification rules that break silently — Turkish amount parsing, Etiket→category precedence, virman/Diğer handling, description casing. Other files cover specific domains: `test_accounts_identity.py` (unique-key enforcement, subtype interest rate), `test_asset_domain.py` (Asset/Investment sync, backfill idempotency), `test_bank_import_deduplication.py` (movement-identity dedup), `test_credit_payments.py` / `test_statements.py` (overlap checks, cascade deletes, archive-without-stealing), `test_recurring_due_dates.py` (weekend/holiday roll-forward), `test_statement_mappings.py` (Etiket rule columns + precedence), `test_tcmb_currency_rates.py` (rate fallback), `test_transaction_update.py`.
...
| `services/recurring.py` | `roll_forward_due_dates(db, owner_id=None)` — `RecurringExpense.next_due` is computed server-side from the record's `frequency`/`day_of_month` and `weekend_rule` (`defer`/`advance`/`none`), with local holidays from Configuration → Local Holidays treated like weekends when `affects_due_dates` is on. It lazily advances stale due dates to the next occurrence on/after today without overrunning `end_date`, and normalizes future monthly dates that were previously saved from the modal start date instead of the payment day. Called at the top of `list_recurring()` (`GET /api/recurring/`) and at the top of `run_due_date_check()`, so every read path self-heals — there is no cron dedicated to this. Also counts the occurrences it steps past and hands them to `apply_recurring_occurrences()` so a prepaid card funding the item is drawn down once per due date. |
...
A **Statement** (`statements` table, `/api/statements`, **Accounts → Statements**) is one uploaded non-credit account statement, kept as an attachment at DB level. It is the **account twin of CreditPayment** — same shape, same lifecycle, different account kind — and the two are mutually exclusive for credit cards by design.

- **A credit-card ekstre produces a CreditPayment, a non-credit account statement produces a Statement — never both for credit cards.** `commit()` in `import.jsx` creates a Statement only for accounts whose type is in `HL_STATEMENTS_API.STATEMENT_TYPES` (`bank`/`overdraft`/`debit`/`wallet`/`cash`/`invest`/`pension`); a `credit` account is handled by the Credit Payment branch. Debit cards remain Statements because they have no payment due date. The Statements API independently rejects `credit` accounts on create/update.
- **Non-card imports default to Wire Transfer only after mapping fails.** `Statement Value Mapping` still wins first, but unmatched rows for accounts other than credit/debit cards become `wire-transfer`. Credit/debit card expenses keep their separate Shopping fallback. Four-character description aliases such as `UNLU` match as whole tokens, so `Günlük` (`gunluk`) does not accidentally classify as Bakery.
- **Deleting a CreditPayment deletes its card spendings.** Rows linked through `transactions.credit_payment_id` are imported statement rows owned by that Card Payment, so `DELETE /api/credit-payments/{id}` removes them together with the record and stored statement file. Non-credit Statement deletion is also account-type aware: `bank`/`overdraft` deletes its linked Account Activity rows, while `invest` deletes that account's holdings and legacy Investment rows.
...
- **Deleting a Statement follows the account domain.** `bank`/`overdraft` removes transactions linked by `statement_id`; `invest` removes every holding and legacy Investment whose platform is that investment account (plus derived `holdings` valuations). Other supported account kinds keep their movements and only clear the statement link. Every branch removes the uploaded file. Deleting the *account* still takes all of its statements with it — see the cascade section below.
- **The import wizard has account-specific entry points, not an Accounts-page modal.** `statements-app.jsx` hosts `ImportWizard` for non-credit accounts and rejects parsed credit-card files; `credit-payments-app.jsx` hosts it with `accountType="credit"`, filters the account matcher to credit cards, and rejects every other parsed account type. Both use the same parser/confirm flow. `Accounts.html` does not load the wizard. A non-credit account detail deep link still navigates to `Statements.html?import=1&account=<account_key>`.
...
- **Transaction imports deduplicate by movement, independent of statement period.** For bank/debit and credit-card imports, `/api/import/confirm` compares owner + resolved account + date + direction + amount + currency + exact description, using occurrence counts so two legitimate identical purchases are preserved. It imports only missing rows; when every reviewed row already exists, the wizard shows **Statement Already Exists** before balances or archive records change. A partial import archives its overlapping statement with `allow_overlap=true`, while relinking claims only previously unlinked movements and never steals rows from an older Statement/CreditPayment. Manual Statement/CreditPayment creates and updates still reject overlapping periods by default; broker portfolio imports keep their period-overlap guard.
...
- **Stale statement due dates are not current debt on Accounts.** If a non-prepaid credit card's stored `payment_due` is more than one month before today, Accounts treats its effective balance as `0` for summary/net totals, group totals, card rows, detail balance, TRY equivalent, and credit-utilization display, and it is excluded from both the Assets and Liabilities balance filters. The stored database balance is left unchanged for auditability.
...
**Almost nothing in the schema points at an account with a foreign key** — the one exception is `Asset.account_id` (see _Assets & Net Worth_). A transaction names its account in `payment_method` as a plain string (the `account_key`, e.g. `acc-12`), a `CreditPayment` carries `account_id` **and** `account_key`, and an invest/pension account's holdings are matched by `Investment.platform == account.name`. So deleting the row alone used to leave those records dangling — visible on Account Activity as a movement whose Account column shows the raw `acc-12` key.
...
- **The cascade also deletes each related `CreditPayment`** (plus its statement file on disk and the spendings pointing at it via `credit_payment_id`), each related **`Statement`** (plus its file — its movements go with the account's other transactions, so a surviving record would point at nothing and still hand out a download), an invest/pension account's holdings, and the account's shadow **`Asset`** row (plus its `AssetValuation` history and any `InvestmentHolding` pointing at it). **Linked accounts survive** — a debit card or overdraft attached to the deleted account keeps existing, only its now-dead `linked_key` is cleared.
- **`DELETE /{acc_id}` returns 200 with a tally** (`transactions, credit_payments, statements, investments, assets, unlinked_accounts`), not 204, so the UI can report what went.
...
- **Accounts detail → View All carries the latest activity period.** The link includes `account`, `year`, and human-facing `month=1..12`; Account Activity validates those query parameters and converts the month to its internal zero-based state. Accounts with no recent activity keep the normal current-month fallback.
...
### Assets & Net Worth (Holdings / Physical Assets)
A parallel `Asset`/`AssetValuation`/`InvestmentHolding` domain (`services/assets.py`, `routers/assets.py`, `routers/holdings.py`, `routers/net_worth.py`) sits **alongside**, not instead of, the older `Account`/`Investment` model — it doesn't replace either, it mirrors them into a valuation-tracked layer for net-worth totals. Frontend: **Assets** group → **Holdings** (`Holdings.html`) and **Physical Assets** (`Assets.html`, id `assets`).

- **Every `bank`/`wallet`/`cash`/`invest`/`pension` `Account` gets a shadow `Asset` row**, kept in sync by `sync_account_domain()` (`ensure_asset_for_account()` + `record_asset_valuation_from_account()`) — `type` maps from the account type (`bank/wallet/cash→liquid`, `invest→investment`, `pension→retirement`), and `valuation_mode` is `"account_balance"` for bank/wallet/cash or `"holdings"` for invest/pension. A `bank`/`overdraft`/`credit`/`debit` account with no ASSET_ACCOUNT_TYPES match gets no shadow `Asset`.
- **`Investment` rows (Midas, BES funds) get a shadow `InvestmentHolding`** via `sync_investment_holding()`, linked back to the source row by `legacy_investment_id` and to a platform-level `Asset` resolved by `ensure_asset_for_platform()` (matches an existing `invest`-type `Account` by name first, else finds/creates a bare `Asset` with `type:"investment"`). Deleting the `Investment` deletes its `InvestmentHolding` via `delete_investment_holding()`.
- **A `Physical Asset` is an `Asset` with `type:"physical"` and `account_id IS NULL`** — a hand-entered item (real estate, vehicle, gold) with no linked account, valued manually via its own `AssetValuation` rows. `routers/assets.py`'s `/physical` sub-routes are scoped to exactly this shape (`_physical_asset()` filters on both conditions); `_physical_data()` forces `account_id=None`, `type="physical"`, `valuation_mode="manual"` on every create/update so a physical asset can never drift into looking account-linked.
- **`AssetValuation` is an append-only time series**, not a mutated field — `record_asset_valuation_from_account()`/`record_asset_valuation_from_holdings()` upsert **one row per `(asset_id, valued_at, source)`** (today's date), so re-running a sync the same day updates that day's row instead of piling up duplicates. `latest_asset_valuation()` (most recent `valued_at`, then `id`) is what `net_worth.summary()` and `_with_latest()` read.
- **`GET /api/net-worth/summary` is TRY-only and additive**: sums every account's positive balance (converted via `amount_to_try()`, which prefers a per-currency `CurrencyRate.to_try` override over the TCMB `ExchangeRate` table) plus every included physical asset's latest valuation (scaled by `ownership_percentage`). It does **not** currently net out liabilities (negative credit-card balances) — `assets_try` and `net_worth_try` are the same number today.
- **`legacy_investment_id`** on `InvestmentHolding` is what keeps this an additive mirror rather than a migration: the original `Investment`/`Account` rows and every existing endpoint that reads them (`AccountHoldings` panel, `/api/investments`) keep working unchanged.
...
|---|---|---|---|
| `User` | `users` | email, full_name, hashed_password, username, role, is_active, avatar_path, avatar_token, language, color | Owner of all other records. `avatar_path`/`avatar_token` back the Profile picture (token = the public capability URL, rotated per upload); `language` is a stored preference with no i18n layer behind it yet — see _Profile (self-service) vs. Members (admin)_. `color` (e.g. `var(--accent)`) is the member's Payer/Paying For display color, editable via Configuration → Members and consumed by every payer/beneficiary UI (`components.jsx` `payerColor()`, calendar chips, report bars) through `window.LEDGER.PAYER_COLORS` |
...
| `Statement` | `statements` | owner_id, account_id, account_key, name, period_year, period_month, period_from, period_to, currency, money_in, money_out, closing_balance, bank_detected, file_path/file_filename/file_mime | One uploaded **non-credit account** statement kept as an attachment; groups the movements in its period (`transactions.statement_id`). Credit cards use `CreditPayment`; debit cards remain valid Statements |
...
| `Asset` | `assets` | owner_id, account_id (nullable), name, type (`liquid`\|`physical`\|`investment`\|`retirement`\|`other`), valuation_mode (`account_balance`\|`holdings`\|`manual`), ownership_percentage, include_in_net_worth | Shadow row mirroring an `Account` or a hand-entered physical item into the net-worth layer — see _Assets & Net Worth_ |
| `AssetValuation` | `asset_valuations` | asset_id, value, currency, valued_at, source, value_try | Append-only time series, one row per `(asset_id, valued_at, source)`; `latest_asset_valuation()` reads the newest |
| `InvestmentHolding` | `investment_holdings` | owner_id, asset_id, legacy_investment_id (nullable), symbol, quantity, average_cost, current_price | Shadow row mirroring a Midas/BES `Investment` into the net-worth layer |
...
| POST | `/` | Create a record. Rejects `credit` accounts; `name` is generated as `"YYYY.MM - Account Name"`; `account_key` is backfilled from `account_id`; the period window is linked to matching movements |
| PATCH | `/{st_id}` | Update. Rejects changing the record to a `credit` account, re-generates the name, and re-links movements when the account or period window changed |
...
### Card Payments — `/api/credit-payments`
Credit-card statement/payment records (**Card Payments**). Deleting a record removes its stored statement file and every spending linked by `transactions.credit_payment_id`.
| Method | Path | Description |
|---|---|---|
| GET | `/` | List credit-card statements, newest due date first; each row carries `linked_count` |
| POST | `/` | Create a Card Payment and link matching card spendings by account + cutover window |
| PATCH | `/{cp_id}` | Update and re-link spendings when account or cutover changes |
| DELETE | `/{cp_id}` | Delete the Card Payment, stored file, and all linked spendings |
| POST | `/{cp_id}/statement/preview` | Attach a supported statement file and return parsed rows for review |
| POST | `/{cp_id}/statement/confirm` | Save reviewed rows as spendings linked to this Card Payment |
| GET | `/{cp_id}/statement` | Download the stored statement file |
...
The table's leading **No.** column displays `StatementMapping.id` as `#<id>`, matching the record number named by normalized-duplicate API errors.
| Method | Path | Description |
|---|---|---|
| GET | `/` | List Etiket/keyword→category mappings (seeded defaults) |
| POST | `/` | Add mapping (`lang`, comma-separated `etiket`, `category_key`, `match_scope`, `priority`, `is_active`) |
...
### Assets — `/api/assets` and `/api/holdings`
See _Assets & Net Worth_ under Architecture.
| Method | Path | Description |
|---|---|---|
| GET | `/api/assets/` | List all of the current user's `Asset` rows (account-linked and physical), each with `latest_valuation` |
| GET/POST/PATCH/DELETE | `/api/assets/physical`, `/physical/{id}` | CRUD for physical assets only (`type:"physical"`, `account_id NULL`) |
| GET/POST | `/api/assets/{id}/valuations` | List/add an `AssetValuation` for any owned asset (account-linked or physical) |
| PATCH/DELETE | `/api/assets/{id}` | Update/delete an account-linked or bare investment `Asset` (not the `/physical` shape) |
| POST | `/api/assets/{id}/refresh-holdings` | Recompute an investment asset's valuation from its current `InvestmentHolding` rows |
| GET/POST/PATCH/DELETE | `/api/holdings/` | CRUD for `InvestmentHolding` rows; `GET` filters by `asset_id`, `account_id`, or `legacy_platform`. Every write recomputes the owning asset's valuation via `record_asset_valuation_from_holdings()` |
| GET | `/api/net-worth/summary` | TRY-only net worth: sum of positive account balances + included physical-asset valuations (see caveat on liabilities in _Assets & Net Worth_) |

### UI Logs — `/api/ui-logs`
| Method | Path | Description |
|---|---|---|
| POST | `/` | Append one client-side log line (level/message/page/path) to a rotating file (`logs/ui-messages.log`, 10 MB × 5 backups) — for capturing frontend errors that never reach the backend otherwise |
...
- One `.html` page per screen (Card Types replaces the older separate Credit/Debit Cards config screens).
...
- Before any visual/UI change, review `DESIGN.md` and keep the implementation aligned with its tokens, spacing, responsive layout, form/modal patterns, table/list controls, and Spending screen reference.
...
| Group | Items |
|---|---|
| Transactions | Spending · Recurring · Card Payments |
| **Assets** (`id: accounts`) | Accounts · Holdings · Account Activity · Physical Assets · Statements |
| Configuration | Members · Categories · Currencies · Card Types · Account Types · Financial Institutions · Statement Value Mapping · Local Holidays · Notifications · Backup & Export |

Account Activity sits under **Assets**, not Transactions — it lists an account's own movements. The Accounts list page keeps the id `accounts`, the same id as its **top-level NAV entry** (labelled "Assets"), so landing on `Accounts.html` lights up both the group and its first item. `Subscriptions.html` still exists as a page but is no longer linked from the sidebar.
...
> **Garanti's "Etiket" column is a structured category tag** — when present it supplies an exact mapping candidate (only the category; direction still follows the amount's sign). Billed Bonus PDFs have no Etiket column; `_parse_garanti_cc_pdf` carries standalone section headings such as `Cafe & Restaurant`, `Süpermarket`, and `Fast Food` into following rows as their Etiket. Precedence in `_normalize_row`: `_cc_classify` (virman/teşekkür/devir/BES) → all matching structured Etiket/section and description-keyword candidates ordered by priority, keyword length, then mapping id → bank-Diğer rule → unmatched credit/debit expense `shopping` → sign. This lets a high-priority merchant rule such as `SBX` override a broader low-priority section tag.
>
> **The Etiket/keyword→category map is DB-driven and user-editable** — `StatementMapping` stores `lang`, comma-separated `etiket` aliases, `category_key`, `match_scope` (`tag|description|both`), `priority`, and `is_active`. `load_etiket_map()` flattens aliases into an in-memory ordered rule list once per preview; no SQL substring query runs per transaction. Every mapping/tag/description search uses the same Turkish-safe lowercase canonicalization: `I/İ/ı/i→i`, Turkish diacritics fold to ASCII, and punctuation/spacing differences do not break a match. Higher priority wins, then the longer keyword, then lower mapping id. Short merchant codes such as `SBX` require token boundaries. The frontend map remains a review-time fallback; backend `category_key` wins in `import.jsx`.
...
- **Before changing any visual surface, read `DESIGN.md`.** It is the source for app-wide design standards, including dark/light theme tokens, desktop/mobile layout, buttons, filters, forms, modals, table controls, pagination, and the current Spending screen reference.
- **A list table becomes a compact 2-line CARD on mobile, not a label/value stack.** The reference is the Spending list (`.tx-row` in the 660px block of `styles/tables.css`): a CSS grid — optional checkbox column spanning both rows, then **line 1 = description/identity + amount**, **line 2 = muted metadata such as date, category, paying-for, and payment method** — with the desktop-only columns hidden separately. The default `td::before{content:attr(data-label)}` stack is what you get by *not* doing this, and it is enormous: Credit Payments rendered 9 label/value rows per record, **342px against Spending's ~60px**, until `.cp-row` was given the same card treatment. Columns dropped from the card are not lost — the row is clickable and its detail modal carries the rest, including Edit/Delete, so hide row action buttons on mobile rather than spending a line on them.
